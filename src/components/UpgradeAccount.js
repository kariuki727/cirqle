...
const UpgradeAccount = ({ isOpen, onClose, tier, user }) => {
  ...
  const handleSavePhone = async () => {
    ...
    try {
      await updateDoc(doc(db, 'users', user.uid), { phone: normalizePhoneNumber(phoneNumber) });
      setIsEditing(false);
      setError('');
    } catch {
  ...
  const handlePayment = async () => {
    if (!user) return setError('Please sign in to proceed.');
    if (!showInput) {
      setShowInput(true);
    ...
    try {
      const response = await axios.post(`${apiUrl}/api/stk-push`, {
  ...
  }, [paymentLoading, clientReference, user, phoneNumber, tier, fee, transactionId]);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          ...
          <div className="mb-4">
            ...
            <div className="flex items-center justify-center gap-2">
              ...
              {isEditing ? (
                ...
                <button onClick={() => setIsEditing(true)} disabled={paymentLoading} className={`p-2 rounded ${paymentLoading ? 'bg-gray-400 cursor-not-allowed' : 'bg-primary text-white hover:bg-accent'}`}>
                  <PencilIcon className="h-5 w-5" />
                </button>
              )}
            </div>
            {error && <p className="text-highlight text-sm mt-2">{error}</p>}
          </div>
        )}

        {showSuccessModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => { setShowSuccessModal(false); onClose(); }}>
      ...
      <style jsx>{`
        .bg-highlight { background-color: #34D1CC; }
        .bg-accent { background-color: #FF4F0F; }
